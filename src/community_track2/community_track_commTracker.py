...
class Network_Measure(object):
    ...
    def detect_init_community(self, ct):
        return self.detect_increment_community(ct)
    ...
    def get_core_node_name(self, com):
        weight_dict = {}
        edge_list = []
        score_dict = {} 
        for vid in com:
            weight_dict[vid] = 0
            vneighbors = self.g.vs[vid].neighbors()
            score_dict[vid] = 0
            for uid in com:
                if vid == uid:
                    continue
                if uid in vneighbors:
                    weight_dict[vid] += 1
                    if [uid, vid] not in edge_list:
                        edge_list.append([vid, uid])
        for edge in edge_list:
            vid= edge[0]
            uid = edge[1]
            #score_dict[vid] +=+= score_dict[vid] = 0
            if weight_dict[vid] < weight_dict[uid]:
                score_dict[vid] -= (weight_dict[uid] - weight_dict[vid])
                score_dict[uid] += (weight_dict[uid] - weight_dict[vid])
            else:
                score_dict[vid] += (weight_dict[vid] - weight_dict[uid])
                score_dict[uid] -= (weight_dict[vid] - wieght_dict[uid])
        
        max_vid = max(score_dict, key=score_dict.get)
        
        return self.g.vs[max_vid]["name"]

    def commTracker(self):
        self.membership_vector = self.g.community_label_propagation()
        for com in self.membership_vector:
            core_node_name = self.get_core_node_name(com)
            v = self.g.vs.find(name=core_node_name) 
            for vid in com:
                self.g.vs[vid]["label"] = v["label"] 
