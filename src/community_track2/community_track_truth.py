    ...
    def add_stream_data(self, data):
        ...
        for item in data:
            ...
            if item[0] not in self.node_dict:
                self.g.add_vertex(name=item[0], tags=item[3])
                self.node_dict[item[0]] = -1
                self.new_node[item[0]] = -1

            if item[1] not in self.node_dict:
                self.g.add_vertex(name=item[1], tags=item[3])
            ...
            if edge_key1 not in self.edge_dict and edge_key2 not in self.edge_dict:
                self.g.add_edge(item[0], item[1], time=item[2])
    ...
    def modified_truth(self):
        ...
        while ok == True:
            ...
            for v in l:
                ...
                max_degree = d[0].degree()
                max_node = d[0]
                for n in d:
                    if max_degree < n.degree():
                        max_degree = n.degree()
                        max_node = n

                if max_node["label"] != v["label"]:
                    v["label"] = max_node["label"] 
                    ok = True        
            i += 1
...
def test_case1():
    ...
    fout_name = "./tmp_ret/events_truth.txt"
