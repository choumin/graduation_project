...
class Stream_Data_Tiles5(object):
    ...
    def get_one_edge_inner(self, line):
        ...
        if edge_key1 in self.edge_dict:
            self.edge_dict[edge_key1][3] += 1
            edge = self.edge_dict[edge_key1]
        elif edge_key2 in self.edge_dict:
            self.edge_dict[edge_key2][3] += 1
            edge = self.edge_dict[edge_key2]
        else:
            edge.append(self.node_dict[u])
            edge.append(self.node_dict[v])
            edge.append(self.get_time(line))
    ...
    def is_invalid2(self, line):
        direction = int(line.split(" ")[7])
    ...
    def get_data_segment(self, period):
        ...
        self.edge_list = [] 
        self.edge_dict = {}
        while line != "" and self.is_invalid(line):
            line = self.readline() 
            print line, "jaja"
        if line == "":
            return []
        ...
        while line != "" and time < begin_time + period:
            line = self.readline()
        ...
        for key in self.edge_dict:
            self.edge_list.append(self.edge_dict[key])
    ...
    def get_community_set(self):
        ...
        for label in community_set_given:
            ...
            subgraphs = self.get_subgraphs(tmp_community)
            for subgraph in subgraphs:
                communities[i] = subgraph
                i += 1
    ...
    def get_time2(self, line):
        return int(line.strip().split(" ")[17]) / 1000
    ...
    def get_label2(self, line):
        direction = int(line.split(" ")[7])
        port1 = int(line.split(" ")[2])
        port2 = int(line.split(" ")[3])
        if direction == 1:
            label = port2
        elif direction == 2:
            label = port1
    ...
    def update_label_dict(self, edge, label):
        ...
        if u not in self.label_dict:
            self.label_dict[u] = {}
        if label not in self.label_dict[u]:
            self.label_dict[u][label] = 0
        self.label_dict[u][label] += 1

        if v not in self.label_dict:
            self.label_dict[v] = {}
        if label not in self.label_dict[v]:
            self.label_dict[v][label] = 0
        self.label_dict[v][label] += 1
